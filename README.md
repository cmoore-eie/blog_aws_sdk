...
This compiles the client into a jar located at `./target/blog-reference-data-sdk-1.0-SNAPSHOT.jar`. Note that this jar does not include any dependencies.
...
```perl
dependencies {
    compile 'blog-reference-data:blog-reference-data-sdk:1.0-SNAPSHOT'
}
```
...
<dependencies>
    <dependency>
        <groupId>blog-reference-data</groupId>
        <artifactId>blog-reference-data-sdk</artifactId>
...
This will compile the client and package all dependencies into a jar located at `./target/blog-reference-data-sdk-1.0-SNAPSHOT.jar`.
...
```java
BlogReferenceDataClientBuilder builder = BlogReferenceData.builder();
```
...
```java
BlogReferenceData client = BlogReferenceData.builder()
...
```java
BlogReferenceData client = BlogReferenceData.builder().build();
client.shutdown();
...
## Making requests
After a client is configured and created, you can make a request to the service. A method on the client interface (`BlogReferenceData`) is created for all actions (resource + method) in your API.
...
```java
BlogReferenceData client = BlogReferenceData.builder().build();
DeleteBranchResult result = client.deleteBranch(new DeleteBranchRequest());
```
...
```java
BlogReferenceData client = BlogReferenceData.builder().build();
client.deleteBranch(new DeleteBranchRequest().sdkRequestConfig(
...
Service exceptions and client exceptions can be handled separately. Any exceptions modeled in the API will be a subtype of BlogReferenceDataException.
```java
try {
    client.deleteBranch(...);
} catch(BlogReferenceDataException e) {
   // All service exceptions will extend from BlogReferenceDataException.
   // Any unknown or unmodeled service exceptions will be represented as a BlogReferenceDataException.
